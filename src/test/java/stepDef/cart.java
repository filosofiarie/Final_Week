...
public class cart extends env {
    ...
    public void user_add_first_product_to_cart() {
        List<WebElement> btn_addToCart = driver.findElements(this.elementProduct.getBtn_addToCart());
        ((WebElement)btn_addToCart.get(0)).click();
        Assert.assertTrue(btn_addToCart.size() == 6);
    }

    @And("user see initial amount")
    public void user_see_initial_amount() {
        WebElement text_amountInCart = driver.findElement(this.elementProduct.getBadge_amountProduct());
        this.prev_amount = Integer.valueOf(text_amountInCart.getText());
    }

    @When("user add second product to cart")
    public void user_add_second_product_to_cart() {
        driver.findElement(this.elementProduct.getBtn_addToCart()).click();
    }
    ...
    public void user_verify_amount_of_product_in_cart_incremented() {
        ...
        Assert.assertTrue(curr_amount - this.prev_amount == 1);
    }
}
